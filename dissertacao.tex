...
A natureza da equação de mapa, descrito na próxima Seção, direciona seu uso para modelagens em que o fluxo está codificado nas relações. Por exemplo, em redes de transporte o fluxo pode se intensificar ou diminuir mas novas rotas não são tipicamente o objeto direto de estudo.
...
% Ronie: faz sentido verificar quantas comunidades possuem ciclos? Provavelmente não agora >_<
...
Após o processo de normalização, ocupações que não possuem ao menos 30 experiências profissionais de currículos diferentes com a mesma ocupação são descartados.

Após a normalização são criados pares e trios de ocupações cronologicamente ordenadas, chamados \textit{bigramas} e \textit{trigramas}. Isso significa que o término de uma ocupação precisa coincidir com o início da seguinte. Uma margem de dois meses de tolerância na sobreposição corrige pequenas imprecisões na anotação dos históricos profissionais. 
...
Os bigramas são usados para a montagem da rede, enquanto os trigramas são usados para a detecção de comunidades. No caso dos trigramas, quando um currículo possui apenas duas experiências profissionais, é gerado um bigrama que é usado também na detecção de comunidades.

A maioria dos trigramas e bigramas de ocupações ocorrem pouquíssimas vezes. Em um conjunto com 4.878.855 sequências distintas, cerca de 4.253.925 ocorrem apenas uma vez, as que se repetem ao menos duas vezes somam 4.666.428 ou 52,31\% do total. Esse grande número de sequências únicas é causada tanto por trajetórias incomuns, quanto por erros de digitação que as etapas anteriores não foram capazes de eliminar. Por vezes são apenas ocupações similares a outras, mas onde os usuários escrevem de maneira pouco convencional. Com alguma frequência essas sequências realmente representam trajetórias em carreiras extremamente específicas.
...
Após o processo de análise, chegou-se a um número próximo a 5 repetições. Sequências com menos de 5 repetições são removidas.

Antes desse processo, cada registro é uma sequência de experiências profissionais. O processo expande cada um em múltiplos registros, representando uma sequência de experiências profissionais de um único indivíduo.
...
\subsection{Identificando Fronteiras de Carreira}  \label{sec:fronteiras-de-carreira}
